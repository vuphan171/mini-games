...
const saveStoreCode = (storeCode: string) => {
  try {
    localStorage.setItem(LOCALSTORAGE_KEYS.STORE_CODE, storeCode);
  } catch (error) {
...
const getStoreCode = () => {
  try {
    return localStorage.getItem(LOCALSTORAGE_KEYS.STORE_CODE) || "";
  } catch (error) {
    console.log("error", error);
    return "";
  }
...
export { saveStoreCode, getStoreCode };
